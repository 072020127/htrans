...
if supports_custom_op():
    ...
    def find_rdma_interfaces():
        rdma_ifaces = []
        for iface in netifaces.interfaces():
            rdma_path = f"/sys/class/net/{iface}/device/infiniband_verbs"
            if os.path.isdir(rdma_path):
                rdma_ifaces.append(iface)
        return rdma_ifaces
    ...
    def barrier_ip(store: Store, world_size: int):
        """
        简易 barrier：每个 rank 对同一个 bar_key 累加一次，
        然后 busy‑wait 直到看到值 == world_size。
        """
        bar_key = "BARRIER_IP"
        if store.add(bar_key, 0) is None:
            store.set(bar_key, b"0")
        store.add(bar_key, 1)
        while int(store.get(bar_key).decode()) < world_size:
            print(f'the {bar_key} is {store.get(bar_key).decode()}')
            pass

    def collect_ip_map(world_size: int, store: Store) -> dict[int, str]:
        """
        读取每个 rank 发布的 IP，返回 {rank: ip}。
        """
        ...
        for r in range(world_size):
            ...
            raw = store.get(key)
            if raw is None:
                raise RuntimeError(f"No IP found for rank {r}")
            mapping[r] = raw.decode("utf-8")
        return mapping
...
class GroupCoordinator:
    ...
    def __init__(
        ...
        if use_device_communicator and self.world_size > 1:
            #logger.info(f"------------ffffffffffffff---------------")
            device_comm_cls = resolve_obj_by_qualname(
                current_platform.get_device_communicator_cls())
            #logger.info(f"------------gggggggggggg---------------")
            #logger.info(f"device_comm_cls: {device_comm_cls}")
            #logger.info(f"cpu_group: {self.cpu_group}, device: {self.device}, "
            ...
            #logger.info(f"------------hhhhhhhhhhhhhhhhhhhhhh---------------")
        #logger.info(f"--------iiiiiiiiiiiiiiiiiiiiiiiiii----------------")
        ...
        #     if self.rank == r:
        #         server = TCPServer("0.0.0.0", server_port)
        #         logger.info("\nserver\n")
        #         def accept_in_bg():
        #             server.register()    # register 包含 accept；会阻塞但在后台线程
        #         t = threading.Thread(target=accept_in_bg, daemon=True)
        #         t.start()
        #         self.servers[r] = server
        #     client = TCPClient(ip, server_port)
        ...
        # for r, client in self.clients.items():
    ...
    def send_object(self, obj: Any, dst: int) -> None:
        ...
        logger.info(" 609 object_tensor = torch.frombuffer(pickle.dumps(obj), dtype=torch.uint8) ")
